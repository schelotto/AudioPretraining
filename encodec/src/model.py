...
class EncodecPretrainingOutput(ModelOutput):
    ...
    Attributes:
        ...
        loss_t (torch.Tensor, optional): Time-domain reconstruction loss.
        loss_f (torch.Tensor, optional): Frequency-domain reconstruction loss.
        loss_g (torch.Tensor, optional): Adversarial loss from the discriminator.
        feat_loss (torch.Tensor, optional): Feature matching loss between real and generated audio.
        loss_w (torch.Tensor, optional): Commitment loss to keep quantized codes close to encoder outputs.
        total_loss (torch.Tensor, optional): Total combined loss.
    """

    audio_values: Optional[torch.Tensor] = None
    audio_codes: Optional[torch.Tensor] = None
    loss_t: Optional[torch.Tensor] = None
    loss_f: Optional[torch.Tensor] = None
    loss_g: Optional[torch.Tensor] = None
    feat_loss: Optional[torch.Tensor] = None
    loss_w: Optional[torch.Tensor] = None
...
class EncodecModelForPretraining(EncodecPreTrainedModel):
    ...
    def forward(
        ...
        if return_loss:
            ...
            total_loss = [loss_t, loss_f, loss_g, feat_loss, loss_w]
            if return_dict:
                ...
                return (total_loss, x_hat) if total_loss is not None else (x_hat, output.audio_codes)
        return output
